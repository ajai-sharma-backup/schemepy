...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        try: 
            symb = self.bindings[symbol]
            return symb.apply() if isinstance(symb, Thunk) else symb
        except KeyError:
            try: return self.parent.lookup(symbol)
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        py_args = []
        while not args.nullp():
            py_args.append(args.car())
            args = args.cdr()
        if self.use_env:
            py_args.append(env)
        try:
...
class LambdaProcedure(Procedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            call_frame = self.env.make_call_frame(self.formals, args)
            return self.body, call_frame
        else:

            call_frame = self.env.make_call_frame(self.formals, args)
            return scheme_eval(self.body, call_frame), None
...
class MuProcedure(LambdaProcedure):
    ...
    def apply(self, args, env):
        if proper_tail_recursion:
            ...
            call_frame = env.make_call_frame(self.formals, args)
            return self.body, call_frame
...
class NuProcedure(LambdaProcedure):
    ...
    def evaluate_arguments(self, arg_list, env):
        return [Thunk(arg, env) for arg in arg_list]
    def apply(self, args, env):
        call_frame = self.env.make_call_frame(self.formals, args)
        return self.body, call_frame
...
class Thunk(LambdaProcedure):
    ...
    # Edited Frame.lookup to make this work
...
def do_lambda_form(vals, env, function_type=LambdaProcedure):
    ...
    body = vals[1] if len(vals.cdr()) == 1 else Pair('begin', vals.cdr())
    return function_type(vals.car(), body, env), None
...
def do_define_form(vals, env):
    ...
    check_form(vals, 2) # Checks that vals is a scheme list with len >= 2
    target = vals[0]
    if scheme_symbolp(target):
        check_form(vals, 2, 2) # Max len = 2
        ...
        if len(vals) == 2:
            lam = LambdaProcedure(target.cdr(), vals.cdr().car(), env)
        else:
            lam = LambdaProcedure(target.cdr(), Pair('begin', vals.cdr()), env)
        do_define_form(Pair(target[0], Pair(lam, nil)), env)
...
def do_let_form(vals, env):
    ...
    while not bindings.nullp():
        names = Pair(bindings.car().car(), names)
        values = Pair(scheme_eval(bindings.car().cdr().car(), env), values)
...
def do_if_form(vals, env):
    ...
    if scheme_eval(vals.car(), env):
        return vals.cdr().car(), env
    else:
        false_suite = vals.cdr().cdr()
        if false_suite.nullp():
            return okay, env
        return false_suite.car(), env

def do_and_form(vals, env):
    """Evaluate short-circuited and with parameters VALS in environment ENV."""

    value = scheme_true
    while not vals.nullp():
        new = scheme_eval(vals.car(), env)
        value = new if value and new else scheme_false
        if not value: break
        vals = vals.cdr()
    return value, None
...
def do_or_form(vals, env):
    ...
    while not vals.nullp():
        new = scheme_eval(vals.car(), env)
        value = new if value or new else scheme_false
        if value: break
        vals = vals.cdr()
    return value, None
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            ...
            if clause.cdr().nullp():
                return scheme_eval(clause.car(), env), None
            elif len(clause.cdr()) == 1:
                return clause.cdr().car(), env
            else:
                return Pair('begin', clause.cdr()), env
...
def do_begin_form(vals, env):
    ...
    last = scheme_eval(vals.car(), env)
    while True:
        if vals.cdr() is not nil:
            vals = vals.cdr()
            last = scheme_eval(vals.car(), env)
        else: return last, None 
...
def check_formals(formals):
    ...
    for i in formals:
        if not i.symbolp():
            raise SchemeError('formals list must only contain symbols')
        if i in seen:
            raise SchemeError('formals list cannot contain duplicates')
        seen.add(i)
